 ...
 * @author r3j20
 */
public class Update_GrowDay_Controller implements Initializable {
    ...
    public static void rxSelPlant(Grow_Data gd) 
     {
        selPlant = gd;
    }
    
    ObservableList<Grow_Data> growData = GrowDataQuery.getAllGrowData();
    ...
    void btnSave(ActionEvent event) throws IOException {
        ...
        if (filled)
            try {
                boolean isFilled = GrowDataQuery.updateGrowDay(
                ...
                if (isFilled) 
                {
                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Grow Day Updated");
                    Optional<ButtonType> result = alert.showAndWait();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            Grow_Data gd = GrowDataQuery.getGrowDataByGrowId(selPlant.getGrowId());
            
            if(gd != null){
                cboPlantId.getSelectionModel().select(Integer.valueOf(gd.getPlantId()));
                cboWater.getSelectionModel().select(gd.getWater());
                cboNut1.getSelectionModel().select(Integer.valueOf(gd.getNutrient1()));
                cboNut2.getSelectionModel().select(Integer.valueOf(gd.getNutrient2())); 
                cboLight.getSelectionModel().select(Integer.valueOf(gd.getLight()));
                cboNewGrowDay.getSelectionModel().select(Integer.valueOf(gd.getGrowDay())); 
                cboDailyHarv.getSelectionModel().select(Integer.valueOf(gd.getDailyHarvest()));
                //Integer.valueOf(txtGrowId.getText());
            ...
            Logger.getLogger(Update_GrowDay_Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
