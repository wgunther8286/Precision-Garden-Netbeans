 ...
 * @author r3j20
 */
public class Update_Plant_Controller implements Initializable {
    ...
    private static Plant selPlant;
    
    public static void rxSelPlant(Plant plant) 
     {
        selPlant = plant;
    }
    ...
    void btnSave(ActionEvent event) throws IOException {
        ...
        {
            try {
                boolean isFilled = PlantQuery.updatePlant(
        ...
        if (isFilled) 
                {
                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Plant Updated");
                    Optional<ButtonType> result = alert.showAndWait();
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR, "Plant not updated");
                    Optional<ButtonType> result = alert.showAndWait();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            Plant plant = PlantQuery.getPlantByPlantID(selPlant.getPlantId());
            
            if(plant != null){
                //for (Plant plant : plant)
                cboPlantType.getSelectionModel().select(String.valueOf(plant.getPlantType()));
                cboPlantSubType.getSelectionModel().select(String.valueOf(plant.getPlantSubType()));
                cboGoal.getSelectionModel().select(String.valueOf(plant.getGoal()));
                cboSTime.getSelectionModel().select(String.valueOf(plant.getStartDate().toLocalTime())); 
                dpSDate.setValue(plant.getStartDate().toLocalDate());
            ...
            Logger.getLogger(Update_Plant_Controller.class.getName()).log(Level.SEVERE, null, ex);
